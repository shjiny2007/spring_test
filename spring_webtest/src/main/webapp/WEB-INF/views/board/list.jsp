   ...
   <c:forEach var="dto" items="${list}">
      <tr>
        ...
        <c:if test="${dto.indent > 0 }">
             <img src="${pageContext.request.contextPath}/images/re.jpg" style = width:20px;height:20px;>
        </c:if>
        <a href="javascript:read('${dto.num}')">${dto.subject}</a>
        
        <c:if test="${util:newImg(dto.regdate)}"> 
        	<img src="${pageContext.request.contextPath}/images/new.gif" style = width:20px;height:20px;>
        </c:if>
        </td>
        <td>${dto.count}</td>
        <td>${dto.regdate}</td>
